#!/usr/bin/env python3
"""
训练Mask R-CNN和Sparse R-CNN模型的脚本
...
def train_mask_rcnn():
    ...
    config_file = "configs/mask_rcnn_coco_simple.py"
...
def train_sparse_rcnn():
    ...
    config_file = "sparse_rcnn_config.py"
...
def main():
    parser = argparse.ArgumentParser(description='训练和测试目标检测模型')
    ...
    if args.model in ['mask_rcnn', 'both']:
        # 查找最佳权重文件
        mask_rcnn_best = "work_dirs/mask_rcnn/best_auto_20240101_000000.pth"  # 实际文件名会根据时间戳变化
        mask_rcnn_latest = "work_dirs/mask_rcnn/latest.pth"
        
        # 优先使用最佳权重，如果不存在则使用最新权重
        if os.path.exists("work_dirs/mask_rcnn/"):
            ...
            elif os.path.exists(mask_rcnn_latest):
                mask_rcnn_checkpoint = mask_rcnn_latest
                print(f"⚠️  未找到最佳权重，使用最新权重: latest.pth")
            else:
                print("❌ Mask R-CNN检查点文件不存在")
                mask_rcnn_checkpoint = None
        else:
            print("❌ Mask R-CNN工作目录不存在")
            mask_rcnn_checkpoint = None
                
        if mask_rcnn_checkpoint:
            test_model("configs/mask_rcnn_coco_simple.py", mask_rcnn_checkpoint, "mask_rcnn")
    ...
    if args.model in ['sparse_rcnn', 'both']:
        ...
        if os.path.exists("work_dirs/sparse_rcnn/"):
            ...
            if sparse_rcnn_checkpoint:
                test_model("sparse_rcnn_config.py", sparse_rcnn_checkpoint, "sparse_rcnn")
